...
createSubgroupCovariateSettings <- function(windowStart = -365, windowEnd = -1, analysisId = 998) {
  ...
  attr(covariateSettings, "fun") <- "ppi30stomachcancer::getDbSubgroupCovariateData"
...
getDbSubgroupCovariateData <- function(connection,
  ...
  writeLines("Creating covariates")
  sql <- SqlRender::loadRenderTranslateSql("CreateSubgroups.sql",
                                           packageName = "ppi30stomachcancer",
  ...
  covariateRef <- data.frame(covariateId = c(1000,2000,3000,4000) + covariateSettings$analysisId,
                             covariateName = c("HP eradication","H2RA","Gastric Ulcer","EGD"),
  ...
  analysisRef <- data.frame(analysisId = as.numeric(covariateSettings$analysisId),
                            analysisName = "interest of Subgroup",
                            domainId = "Cohort",
